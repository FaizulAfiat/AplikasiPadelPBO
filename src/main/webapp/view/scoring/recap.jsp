    ...
    <body class="bg-gray-100 min-h-screen flex flex-col font-sans">
        ...
        <header class="flex border-b border-gray-200 bg-white sticky top-0 z-50">
            ...
            <div class="flex-1 border-r border-gray-200 flex items-center px-8">
                <a href="${pageContext.request.contextPath}/index.jsp" class="text-xs font-bold uppercase tracking-widest text-gray-600 hover:text-black hover:underline transition-colors">← Dashboard</a>
            </div>
            <div class="p-4 md:p-6 w-1/2 md:w-1/4 flex items-center justify-end gap-2">
                <span class="text-[10px] font-bold bg-gray-100 text-gray-800 border border-gray-200 px-3 py-1.5 rounded-full uppercase" id="mode-badge">${scoringStyle}</span>
            </div>
        </header>
        ...
        <div class="flex-1 p-6 pb-20 flex flex-col items-center max-w-4xl w-full mx-auto justify-center">
            ...
            <div class="text-center mb-8">
                <span class="text-xs font-bold bg-gray-100 text-gray-800 border border-gray-200 px-4 py-1.5 rounded-full uppercase tracking-widest">Match #${matchId}</span>
            ...
            <div class="w-full mb-8">
                <c:choose>
                    <c:when test="${winner eq 'TEAM 1 WINS!'}">
                        <div class="border border-cyan-200 p-6 rounded-2xl bg-cyan-50 text-cyan-800 font-bold uppercase italic shadow-sm text-center text-2xl md:text-3xl animate-bounce">
                            🏆 Team 1 Wins!
                        </div>
                    </c:when>
                    <c:when test="${winner eq 'TEAM 2 WINS!'}">
                        <div class="border border-yellow-200 p-6 rounded-2xl bg-yellow-50 text-yellow-800 font-bold uppercase italic shadow-sm text-center text-2xl md:text-3xl animate-bounce">
                            🏆 Team 2 Wins!
                        </div>
                    </c:when>
                    <c:otherwise>
                        <div class="border border-gray-200 p-6 rounded-2xl bg-gray-50 text-gray-800 font-bold uppercase italic shadow-sm text-center text-2xl md:text-3xl">
                            🤝 Draw Match!
                        </div>
                    </c:otherwise>
                </c:choose>
