    ...
    <header class="flex border-b border-grid bg-white sticky top-0 z-50">
        ...
        <div class="flex-1 border-r border-grid hidden md:flex items-center px-8">
            <a href="${pageContext.request.contextPath}/index.jsp"
                class="text-xs font-bold uppercase tracking-widest hover:underline">← Back to Dashboard</a>
        </div>
    ...
    <main class="flex flex-col md:flex-row flex-1">
        ...
        <div class="flex-1 p-8 md:p-12 bg-white">
            ...
            <form action="${pageContext.request.contextPath}/BookingController" method="POST"
                class="max-w-2xl space-y-12" id="bookingForm">
                ...
                <div class="col-span-full">
                    ...
                    <div class="max-h-[550px] overflow-y-auto border border-gray-200 rounded-2xl p-4 bg-gray-50/50 shadow-inner space-y-3" id="time-grid">
                        <c:forEach var="s" items="${timeSlots}">
                            <div class="flex items-center gap-4 group">
                                <!-- Left: Time Indicator -->
                                <div class="w-20 text-right flex-shrink-0">
                                    <span class="text-sm font-black text-gray-800 tracking-tight block">${s.time}</span>
                                    <span class="text-[10px] font-semibold text-gray-400 block">${s.time.plusHours(1)}</span>
                                </div>
                                
                                <!-- Middle line node -->
                                <div class="relative self-stretch flex flex-col items-center">
                                    <div class="w-3 h-3 rounded-full border-2 border-gray-300 bg-white group-hover:border-black transition-colors duration-300 z-10"></div>
                                    <div class="w-[2px] flex-1 bg-gray-200 group-last:bg-transparent"></div>
                                </div>

                                <!-- Right: Interactive Block -->
                                <c:choose>
                                    <c:when test="${s.isAvailable}">
                                        <button type="button" data-time="${s.time}" onclick="handleSelection(this)"
                                            ...
                                            <div class="flex flex-col">
                                                <span class="text-sm font-bold text-gray-800 slot-time-text">${s.time} - ${s.time.plusHours(1)}</span>
                                                <span class="text-[10px] text-gray-400 font-medium uppercase tracking-wider slot-status-hint">Klik untuk memilih</span>
                                            </div>
                                            <span class="text-[10px] font-black px-2.5 py-1 bg-green-50 text-green-600 rounded-full border border-green-200 uppercase tracking-wider slot-badge">
                                                Tersedia
                                            </span>
                                        </button>
                                    </c:when>
                                    <c:otherwise>
                                        <button type="button" disabled data-time="${s.time}"
                                            ...
                                            <div class="flex flex-col opacity-60">
                                                <span class="text-sm font-bold text-gray-500 line-through slot-time-text">${s.time} - ${s.time.plusHours(1)}</span>
                                                <span class="text-[10px] text-red-500 font-semibold uppercase tracking-wider">Sudah Dipesan (Reservasi Aktif)</span>
                                            </div>
                                            <span class="text-[10px] font-black px-2.5 py-1 bg-red-100/80 text-red-600 rounded-full uppercase tracking-wider flex items-center gap-1">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-3.5 h-3.5">
                                                  <path fill-rule="evenodd" d="M8 1a3.5 3.5 0 0 0-3.5 3.5V7A1.5 1.5 0 0 0 3 8.5v5A1.5 1.5 0 0 0 4.5 15h7a1.5 1.5 0 0 0 1.5-1.5v-5A1.5 1.5 0 0 0 12.5 7V4.5A3.5 3.5 0 0 0 8 1Zm2.5 6V4.5a2.5 2.5 0 1 0-5 0V7h5Z" clip-rule="evenodd" />
                                                </svg>
                                                Booked
                                            </span>
                                        </button>
                ...
                <div class="border-b-2 border-black pb-6">
                    <label class="text-xs font-bold uppercase opacity-50 block mb-4">Select Court</label>
                                    ...
                                    <div class="p-4">
                                        <div class="flex justify-between items-center mb-2">
                                            <span
                                                ...
                                                ${court.courtId % 2 == 0 ? 'Outdoor Court' : 'Indoor Court'}
                                            </span>
                                        ...
                                        <p
                                            ...
                                            ${court.courtId % 2 == 0 ? 'Panoramic Sunset • Scenic View • Premium Turf' : 'Pro Blue Turf • Glass Walls • LED Spotlights'}
                                        </p>
                                    </div>
                                </div>
                                <!-- Check badge -->
